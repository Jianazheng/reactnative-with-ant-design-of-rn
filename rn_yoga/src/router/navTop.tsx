...
export default class NavTop extends Component<Props> {
	...
	_renderTouchableHighlight(children, style, title, isioscreen) {
		return (
			<View style={[styles.navtopMain1, mainStyle.jcBetween, mainStyle.aiCenter, mainStyle.bgcfff, isioscreen ? { marginTop: this.state.statusBar } : null]}>
	...
	_renderShowTitle(style, title, isioscreen) {
		return (
			<View style={[styles.navtopMain, mainStyle.jcCenter, mainStyle.bgcfff, isioscreen ? { marginTop: this.state.statusBar } : null]}>
	...
	render() {
		let isioscreen = this.isios;
		let { navType, style, title, children } = this.props;
		switch (navType) {
			case 'normal':
				return this._renderTouchableHighlight(children, style, title, isioscreen);
				break;
			case 'title':
				return this._renderShowTitle(style, title, isioscreen)
				break;
		}
	}
}
